...
class SellViewController: UIViewController, UICollectionViewDelegateFlowLayout {
    ...
    var viewModel: SellViewModeling!
    ...
    func setupBinding(){
      ...
      //sizeTextField.inputView = sizePickerView
        ...
        createListingButton.rx.tap.asObservable()
            .subscribe(onNext: { [weak self] _ in
                if let result = self?.validateAllFields(), result == true{
                    if let count = self?.viewModel.images.count, count > 1 {
                        ...
                        self?.viewModel.submitDidTap.onNext(())
                    } else {
...
//            .disposed(by: disposeBag)
        ...
        viewModel.submissionResult.subscribe(onNext: { [weak self] result in
            ...
            case .submissionSuccess:
                ...
                LoadingManager.shared.hideIndicatorWithMessage(message: "Item uploaded successfully", timeInterval: 2)
...
extension SellViewController: SellViewImageDelegate {
    ...
    func refreshAllData() {
        ...
        emailTextField.text = ""
        emailTextField.placeholder = "PayPal Email"
...
extension SellViewController: TLPhotosPickerViewControllerDelegate{
